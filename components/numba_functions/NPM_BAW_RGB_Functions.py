...
def get_patch_absolute_difference_bak(currentIndex,
    ...
    max_absolute_difference = np.maximum(absolute_difference[:,:,0], np.maximum(absolute_difference[:,:,1], absolute_difference[:,:,2]))
    sum_of_absolute_difference = np.sum(max_absolute_difference)

    return 3*sum_of_absolute_difference/(np.sum(filter))
@jit(nopython=disable_debug)
def get_patch_absolute_difference(currentIndex,
    ...
    spatial_weights = get_spacial_distance_weights(filter)
    single_weights = get_bilateral_suport_weights(max_absolute_difference, gamma_c, gamma_s)
...
if __name__ == "__main__":
    ...
    filter = np.ones((7, 5, 3), dtype=np.int32)
    SimpleTimer.timeit()

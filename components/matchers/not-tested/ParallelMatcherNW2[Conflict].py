...
def getOne(*argvs, **kwargs):
    return ParallelMatcherNW2(*argvs, **kwargs)


class ParallelMatcherNW2(ParallelMatcher, SequentialMatcher, Matcher):

    def __init__(self, match, gap, egap, initIndex=0):
        self.match = match
        self.gap = gap
        self.egap = egap
        self.matrices = None
        self.initFunctions = [self.fillUpFirstRows1, self.fillUpFirstRows2, self.fillUpFirstRows3]

        self.fillUpFirstRows = self.initFunctions[initIndex]
    ...
    def fillUpFirstRows1(self, matrix):
                matrix[0, 1:] = self.gap * self.columns

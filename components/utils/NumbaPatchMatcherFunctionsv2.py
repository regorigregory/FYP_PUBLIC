...
@jit(nopython=disable_debug)
def match_scanlines_maclean(match, gap, egap, current_index, im2_padded, im1_padded, scores, moves,
                    filter=np.ones((3, 3), dtype=np.int32), dmax=256):
    start_x = int((filter.shape[1] - 1) / 2)
    start_y = int((filter.shape[0] - 1) / 2)
    ...
    for i in range(1, im1_scanline.shape[0] - start_x * 2 + 1):
        ...
        for j in range(starting_index, i+1):
            ...
            # match_raw = match - abs(im1_scanline[im1_index] - im2_scanline[im2_index])
            # match_raw =  match - abs(im1_scanline[im1_index] - im2_scanline[im2_index]) #get_patch_absolute_difference(current_index, im1_index, im2_index,im1, im2)
            match_raw = match - get_patch_absolute_difference(current_index, im1_index, im2_index, im1_padded,
                                                              im2_padded, filter)
            east, north, ne = (i, j - 1), (i - 1, j), (i - 1, j - 1)
            east_raw, north_raw, ne_raw = scores[east], scores[north], scores[ne]
            east_dir, north_dir, ne_dir = moves[east], moves[north], moves[ne]
...
@jit(nopython=disable_debug, parallel = disable_debug)
def match_images(match, gap, egap, im1, im2, scores_raw, moves_raw, scores_n_moves,
                 disparity, scanline_match_function, filter = np.ones((5,5), dtype=np.int32)):
...
@jit(nopython=disable_debug)
def test_pipeline(match, gap, egap, im1, im2, scanline_match_function=match_scanlines_maclean, filter = np.ones((3,3), dtype=np.int32)):
...
if __name__ == "__main__":
    ...
    SimpleTimer.timeit()
    x,z = test_pipeline(match, gap, egap, im1, im2, filter = np.ones((1,1), dtype = np.int32))

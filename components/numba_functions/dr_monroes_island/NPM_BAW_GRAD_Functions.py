...
def get_patch_absolute_difference(currentIndex,
    ...
    if cache_left[key_left, 0, 0] == np.inf:
        ...
        p1_weights = cf.get_bilateral_suport_weights(patch1, gamma_c, gamma_s) if product_flag else cf.get_bilateral_suport_weights_sum(patch1, gamma_c, gamma_s)
        cache_left[key_left] = p1_weights * grad1
    ...
    if cache_right[key_right, 0, 0] == np.inf:
        ...
        p2_weights = cf.get_bilateral_suport_weights(patch2, gamma_c, gamma_s) if product_flag else cf.get_bilateral_suport_weights_sum(patch2, gamma_c, gamma_s)
    ...
    return min(term_to_be_truncated, T_c)
...
if __name__ == "__main__":
    ...
    SimpleTimer.timeit()
    x, z = test_pipeline(match, gap, egap, im1, im2, filter=np.ones((3, 3), dtype=np.float64), gamma_c=3, gamma_s=2,
                         alpha=0.12, product_flag=False)

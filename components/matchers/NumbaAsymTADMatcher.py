...
from components.numba_functions import temp as patch_functions
...
if __name__ == "__main__":
    ...
    im1_path = os.path.join(project_helpers.get_project_dir(), "datasets", "middlebury", "middlebury_2003", "cones",
                            "im2.png")
    im2_path = os.path.join(project_helpers.get_project_dir(), "datasets", "middlebury", "middlebury_2003", "cones",
                            "im6.png")
    scaler = 256
    left = cv2.imread(im1_path, cv2.IMREAD_GRAYSCALE).astype(np.float64)/scaler
    right = cv2.imread(im2_path, cv2.IMREAD_GRAYSCALE).astype(np.float64)/scaler

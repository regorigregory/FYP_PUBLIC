...
def get_patch_absolute_difference(currentIndex,
    ...
    if cache_right[key_right, 0, 0] == np.inf:
        ...
        p2_weights = cf.get_bilateral_suport_weights(patch2, gamma_c, gamma_s)
...
def match_images(match, gap, egap,
    ...
    for i in prange(im1.shape[0]):
        ...
        for c in range(3):
            ...
            max_value_temp = np.max(scores_temp[-1, :])
...
if __name__ == "__main__":
    ...
    SimpleTimer.timeit()
    x, z = test_pipeline(match, gap, egap, im1, im2, filter=np.ones((7, 3), dtype=np.float64), gamma_c=10, gamma_s=100,
                         alpha=0.0)

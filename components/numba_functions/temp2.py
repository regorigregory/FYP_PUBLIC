...
def calculate_gradients_de_maetzu(img_padded):
    ...
    for i in prange(1, int(img_padded.shape[0] - 1)):
        for j in prange(1, int(img_padded.shape[1] - 1)):
            ...
            grads[i + 1, j + 1] = np.sqrt(G_x ** 2 + G_y ** 2)
            phase[i + 1, j + 1] = np.arctan(G_y / G_x)
    return grads, phase
...
def match_images(match, gap, egap,
    ...
    gcl, pl = calculate_gradients_de_maetzu(im1_padded)
    gcr, pr = calculate_gradients_de_maetzu(im2_padded)
...
if __name__ == "__main__":
    if __name__ == "__main__":
        ...
        match = 1.57439627e+01 / scaler  # 20.08/(256)

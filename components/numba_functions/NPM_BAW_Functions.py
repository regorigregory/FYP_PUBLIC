...
def calculate_gradients(img_padded):
    ...
    for i in prange(1, img_padded.shape[0]):
        ...
        for j in prange(1, img_padded.shape[1]):
            ...
            grads[i,j] = np.sum(current_row[current_indices]*grad_filter)/2
    return grads
...
@jit(nopython=disable_debug)
def get_patch_absolute_difference_bak(currentIndex,
    ...
    absolute_difference = np.abs(left_window*patch1-right_window*patch2)
    ...
    if(alpha>0):
        ...
        sum_of_absolute_grad_difference = np.abs(left_window*grad_left-right_window*grad_right)
        sum_of_absolute_grad_difference = sum_of_absolute_grad_difference/sum_of_absolute_grad_difference.max()
        sum_of_absolute_grad_difference = sum_of_absolute_grad_difference*absolute_difference.max()
        sum_of_absolute_grad_difference = np.sum(sum_of_absolute_grad_difference)*alpha
...
if __name__ == "__main__":
    ...
    SimpleTimer.timeit()
    x,z = test_pipeline(match, gap, egap, im1, im2, filter = np.ones((7,5), dtype = np.int32), gamma_c=10, gamma_s=90, alpha=0.0)
